...
Last verified: 2026-06-22 00:41 Europe/Paris.
...
- Bitcoin Core is now out of Initial Block Download on a fresh pruned volume. Live checks on 2026-06-21 23:03 Paris time showed `blocks=954742`, `headers=954742`, `initialblockdownload=false`, and `pruned=true` with a `55 GiB` prune target on a `90 GiB` local PV.
...
- BTCPay LTC is public through Greenfield invoice creation. A live production smoke test on 2026-06-21 22:49 Paris time returned invoice `JqMUzR5VYX1qgoVxqRXuK3` with an `LTC-CHAIN` payment link for `0.0255037 LTC` at rate `39.21`.
- BTCPay BTC is public-ready through Greenfield invoice creation. On 2026-06-21 23:01 Paris time, the Marky store received a generated `BTC-CHAIN` watch-only wallet; the secret mnemonic backup was written only to local ignored material under `.secrets/`. A live production smoke test returned invoice `RnwkUqnEZ6sWkryHD9cob2` with a `BTC-CHAIN` payment link for `0.00001797 BTC` at rate `55650`. This is the prerequisite for setting `BTCPAY_BTC_WALLET_READY=true` and `NEXT_PUBLIC_BTCPAY_BTC_WALLET_READY=true`.
- The storefront production secret contains Stripe Payment Links, Solana Pay settings and BTCPay env vars. Public selling is controlled by `SALES_ENABLED`, not by secrets simply existing. As of 2026-06-22, `CONSUMER_MEDIATOR_NAME` and `CONSUMER_MEDIATOR_WEBSITE` are advisory disclosure fields only; the owner explicitly accepts launching without a consumer-mediator gate.
...
- The storefront has the USDC on Solana Pay flow implemented behind the sales gate: `POST /api/checkout/stablecoin`, the internal page `/checkout/stablecoin`, `POST /api/checkout/stablecoin/verify`, optional `POST /api/webhooks/shkeeper`, and shared PostgreSQL reconciliation through `creator_orders`. Solana Pay verification supports `SOLANA_PAY_RPC_URLS` as a comma-separated read-only RPC fallback list. USDC invoice pricing uses the free Frankfurter/ECB EUR to USD rate first and falls back to `STABLECOIN_EUR_TO_USD_RATE` only when that optional env value is configured. New Solana Pay invoices expire through `SOLANA_PAY_INVOICE_TTL_MINUTES` so EUR/USD quotes are not left payable indefinitely.
...
- BTC/LTC checkout is constrained per invoice through BTCPay Greenfield `checkout.paymentMethods`, matching the official schema. The storefront only sends payment method IDs that have passed smoke tests: `LTC` and `BTC`. The runtime BTCPay API key stays least-privilege for the site and does not need store-settings read access.
...
| BTC on-chain via BTCPay | Live and smoke-tested | Self-hosted crypto, non-custodial checkout | Low during calm mempool, slower settlement | Bitcoin wallet backup and invoice smoke | Public UTXO rail alongside LTC |
...
BTC is public-ready as of 2026-06-21 23:03 Europe/Paris. Keep it public only while these checks continue to pass:
...
- A test invoice can be created with a `BTC-CHAIN` destination, payment link, exchange rate and amount due.
...
- `CONSUMER_MEDIATOR_NAME` and `CONSUMER_MEDIATOR_WEBSITE` are advisory
  disclosure fields only. They do not gate `SALES_ENABLED=true`; keep the legal
  pages accurate and update them if a referenced mediator is contracted later.
