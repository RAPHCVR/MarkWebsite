...
- The storefront production secret contains Stripe Payment Links, Solana Pay settings and BTCPay env vars. Public selling is controlled by `SALES_ENABLED`, not by secrets simply existing. As of 2026-06-22 00:41 Paris time, public checkout is intentionally disabled because no referenced consumer mediator is configured yet; the payment rails stay ready/configured, but the site must keep the legal gate closed until `CONSUMER_MEDIATOR_NAME` and `CONSUMER_MEDIATOR_WEBSITE` contain real contracted/referenceable values.
