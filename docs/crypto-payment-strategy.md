...
Last verified: 2026-06-21 13:58 Europe/Paris.
...
- Bitcoin Core is in Initial Block Download on a fresh pruned volume. Live checks on 2026-06-21 13:58 Paris time showed the node advancing around block `887354` of `954694`, `verificationprogress=0.862084`, `initialblockdownload=true`, and `pruned=true` with a `55 GiB` prune target on a `90 GiB` local PV. During IBD, BTCPay returns `{"synchronized":false}` and BTC checkout must remain disabled.
...
- A production smoke test on 2026-06-21 13:58 Paris time via `scripts/audit-payment-readiness.ps1 -RunBtcpaySmoke -RunStablecoinSmoke` returned `PASS` after the live Deployment was moved to image digest `6d5f3422...`: Stripe readiness, USDC Solana Pay readiness, BTCPay config, public BTCPay checkout, Litecoin sync, BTCPay LTC payment method generation, and USDC unpaid pending verification. It created BTCPay invoice `CWRruGt23et4pnowtfNgvU`, confirmed an `LTC-CHAIN` payment link for `0.02552323 LTC` at rate `39.18`, and cleaned the Solana Pay smoke order row.
